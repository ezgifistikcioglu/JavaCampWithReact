...
export default function UpdateSocialMediaForCv() {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      values.id = parseInt(4)
      console.log("values: " + values)
      values.cvId = parseInt(13);

      socialMediaService.updateSocialMedias(values).then((result) => console.log(result)).then(swal({
        title: "Succeed!",
        text: "Social Media is updated!",
        icon: "success",
        button: "Ok"
  ...
  return (
    <div>
      <Card color="orange" fluid>
        <Card.Content header='Update Social Media' />
        <Card.Content>
